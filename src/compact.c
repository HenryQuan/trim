...
#define MIN_LEN    8            /* catch genuinely-reused short phrases like "pub(crate) fn " */
...
/* find the best repeated substring (len>=minlen, occ>=2) in unused regions.
   start must follow a separator; the end is the natural longest-common-prefix cut
   where the occurrences diverge. writes the value to out (<=outcap), returns len (0=none). */
static int find_repeated(const char *s, size_t n, const unsigned char *used,
    ...
    for (size_t li = 0; li < sizeof(LENS) / sizeof(LENS[0]); li++) {
        ...
        while (i < cnt) {
            ...
            if (j - i + 1 > 4096) { i = j + 1; continue; }   /* degenerate (all-same text) */
            int p0 = rec[i].pos;
            if (p0 > 0 && !is_boundary(s[p0 - 1])) { i = j + 1; continue; }
            if (s[p0] == ' ' || s[p0] == '\t') { i = j + 1; continue; }   /* no leading whitespace */
            if (!region_free(used, p0, L)) { i = j + 1; continue; }

            int minlcp = (int)n - p0, occn = 0, last = -1;
            for (int k = i; k <= j; k++) {
                int pos = rec[k].pos;
                if (last >= 0 && pos < last) continue;
                if (pos > 0 && !is_boundary(s[pos - 1])) continue;
                if (!region_free(used, pos, L)) continue;
                if (memcmp(s + pos, s + p0, (size_t)L) != 0) continue;   /* hash collision guard */
                int c = 0;
                while (c < minlcp && c < 512 && pos + c < (int)n && s[pos + c] == s[p0 + c]) c++;
                if (c < minlcp) minlcp = c;
                occn++;
                last = pos + c;
            }
            if (occn >= 2 && minlcp >= minlen) {
                ...
                if (Lv >= minlen) {
                int sav = occn * (Lv - 2);   /* body savings only; the small dict header is not a cost */
                if (sav > best) { best = sav; bestocc = occn; bestlen = Lv; bestpos = p0; }
                }
            }
            i = j + 1;
        }
...
static int count_sub(const char *hay, const char *needle) {
    ...
    for (size_t i = 0; i + nn <= hn; ) {
        if (memcmp(hay + i, needle, nn) == 0) { c++; i += nn; }
        else i++;
    }
    return c;
}

/* build the body of s with every dict value replaced by its $N reference (longest first) */
static char *build_body(const char *s, size_t n, char valbuf[][257], int vlen[], int ndict) {
    int order[MAX_DICT];
    ...
    for (size_t i = 0; i < n; ) {
        ...
        for (int oi = 0; oi < ndict; oi++) {
            int d = order[oi];
            if (i + (size_t)vlen[d] <= n && memcmp(s + i, valbuf[d], (size_t)vlen[d]) == 0) { m = d; break; }
        }
        if (m >= 0) {
            char ref[12];
            snprintf(ref, sizeof(ref), "$%d", m + 1);
            size_t rl = strlen(ref);
            if (on + rl + 2 >= cap) { cap *= 2; res = realloc(res, cap); }
            memcpy(res + on, ref, rl); on += rl;
            i += (size_t)vlen[m];
        } else {
...
/* lossless reference compaction: $1..$N = repeated substrings, body substitutes refs.
   greedy by real savings; a post-pass drops any ref used fewer than 2 times. */
char *compact_paths(const char *s, size_t *out_n) {
    ...
    /* post-pass: drop refs that appear fewer than 2 times in the final body (a net loss) */
    char *body = NULL;
    for (;;) {
        if (body) free(body);
        ...
        for (int d = 0; d < ndict; d++) {
            ...
            if (count_sub(body, ref) < 2) { bad[d] = 1; nbad++; }
        }
        if (nbad == 0) break;
        int w = 0;
        for (int d = 0; d < ndict; d++) if (!bad[d]) {
            if (w != d) { memcpy(valbuf[w], valbuf[d], 257); vlen[w] = vlen[d]; }
            w++;
        }
        ndict = w;
        if (ndict == 0) { free(body); body = NULL; break; }
    }
    ...
    if (on + blen + 2 >= cap) { cap = on + blen + 64; res = realloc(res, cap); }
...
/* shared for rg / sg / fd: compact output, cap only as a safety net (line-aligned) */
void cmd_search(char **args, int argc) {
    ...
    if (cn > SEARCH_CAP) {
        ...
        if (cut == 0) cut = SEARCH_CAP;   /* no newline within cap; hard cut */
